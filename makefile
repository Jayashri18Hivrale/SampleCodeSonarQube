...
SRC_DIRS += $(WORKSPACE_PATH)/src/chipDrivers
SRC_DIRS += $(WORKSPACE_PATH)/src/common
SRC_DIRS += $(WORKSPACE_PATH)/src/periphdrivers
SRC_DIRS += $(WORKSPACE_PATH)/tests/stubs


# SRC_FILES += add source files
...
# TEST_SRC_DIRS += tests/srctest
TEST_SRC_DIRS += $(WORKSPACE_PATH)/tests/helpers
TEST_SRC_DIRS += $(WORKSPACE_PATH)/tests/test_cases
...
INCLUDE_DIRS += $(WORKSPACE_PATH)/include/common
INCLUDE_DIRS += $(WORKSPACE_PATH)/include/periphdrivers
INCLUDE_DIRS += $(WORKSPACE_PATH)/tests/helpers/
INCLUDE_DIRS += $(WORKSPACE_PATH)/tests/helpers/include
INCLUDE_DIRS += $(WORKSPACE_PATH)/tests/stubs/
INCLUDE_DIRS += $(WORKSPACE_PATH)/tests/mocks/
...
CPPUTEST_CFLAGS += -D__TDD_CPPUTEST__ 
CPPUTEST_CPPFLAGS += -D__TDD_CPPUTEST__ 
...
CPPUTEST_CXXFLAGS += -Wno-extra-tokens
endif
...
CPPUTEST_WARNINGFLAGS += -Wno-unknown-pragmas


CPPUTEST_CFLAGS += -pedantic
CPPUTEST_CFLAGS += -Wno-missing-prototypes
CPPUTEST_CFLAGS += -Wno-strict-prototypes
CPPUTEST_CFLAGS += -Wno-extra-tokens
CPPUTEST_CXXFLAGS += -Wno-c++14-compat
