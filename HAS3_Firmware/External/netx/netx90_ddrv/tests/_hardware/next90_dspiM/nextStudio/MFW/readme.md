...
The maintainance firmware version is documented in the following subsection.
...
Please refer to the Hilscher Knowledge base http://kb.hilscher.com for the latest version.

This file should not be modified and should be used as provided, the maintenance firmware is an update/configuration firmware.

The communication-side firmware uses an MFW file that is installed in the flash memory. The extension of this filename is `.mxf`. Note that for some reason, the file extension does not match the given acronym. It's important to note that throughout the documentation, this file is referred to as either MXF or MFW.
